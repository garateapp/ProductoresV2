...
<body>
    ...
    <script>
        ...
        function generateCalibreLegend(labels, counts, legendContainerId) {
            ...
            const formatter = (typeof Intl !== 'undefined') ? new Intl.NumberFormat('es-CL') : null;
            ...
            safeLabels.forEach((label, index) => {
                ...
                const count = formatter ? formatter.format(rawValue) : rawValue;
                ...
                @if ($recepcion->n_especie === 'Cherries')
                    @php
                        ...
                        foreach ($groupedRows as $group => $rows) {
                            $groupTotals[$group] = array_sum(array_map(static function ($row) {
                                return $row['total'];
                            }, $rows));
                        }
                    @endphp
                            ...
                            <table class="color-calibre-matrix">
                                        ...
                                        @if (!empty($groupedRows[$orderGroup]))
                                            @foreach ($groupedRows[$orderGroup] as $index => $row)
                                                <tr>
                                                    ...
                                                    @foreach ($calibres as $calibre)
                                                        <td>{{ number_format($row['counts'][$calibre] ?? 0, 0, ',', '.') }}</td>
                                                    @endforeach
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        @foreach ($calibres as $calibre)
                                            <th>{{ number_format($columnTotals[$calibre] ?? 0, 0, ',', '.') }}</th>
                                        @endforeach
                                        <th>{{ number_format($overallTotal, 0, ',', '.') }}</th>
                                    </tr>
                                </tfoot>
                            </table>
                            <div class="color-matrix-summary">
                                @foreach ($groupOrder as $orderGroup)
                                    @if (!empty($groupTotals[$orderGroup]))
                                        <span><strong>{{ $orderGroup }}:</strong> {{ number_format($groupTotals[$orderGroup], 0, ',', '.') }}</span>
                                    @endif
                                @endforeach
                            </div>
                        @else
