...
const TYPES = [
  { value: 'principal_1', label: 'Principal 1' },
  { value: 'principal_2', label: 'Principal 2' },
];
...
export default function Index({ auth, versions, seasons, statuses, filters }) {
  ...
  }, [data.season_id, data.type, data.status]);
  ...
  const statusOptions = [{ value: '', label: '(Todos)' }, ...statuses.map(s => ({ value: String(s.name), label: s.name }))];

  const selectedSeasonId = importForm.data.season_id || data.season_id;
  const selectedType = importForm.data.type || data.type || 'principal_1';

  const handleDownload = () => {
    if (!selectedSeasonId || !selectedType) return;
    const params = {
          ...
          <CardContent>
            ...
            >
              ...
              <div>
                ...
                <select value={importForm.data.type} onChange={(e) => importForm.setData('type', e.target.value)} className="border rounded px-2 py-2">
                  {TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
                {importForm.errors.type && <div className="text-red-600 text-sm">{importForm.errors.type}</div>}
              </div>
              <div>
              ...
              <Button type="submit" disabled={importForm.processing}>Subir CSV</Button>
            </form>
              ...
              <TableBody>
                {versionRows.map((v) => (
                  <TableRow key={v.id}>
                    ...
                    <TableCell>
                      <Link href={route('estimations.show', v.id)} className="text-indigo-600 hover:underline">Ver</Link>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
