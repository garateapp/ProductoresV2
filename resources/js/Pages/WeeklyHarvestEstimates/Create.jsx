...
export default function Create({ auth, especies, producers, defaults }) {
  ...
  const { data, setData, post, processing, errors } = useForm({
    ...
    especie_id: '',
    variedad_id: '',
    season_code: defaults.season_code,
    iso_year: defaults.iso_year,
    iso_week: defaults.iso_week,
    predio: '',
    block: '',
    estimated_kilos: '',
    estimated_bins: '',
  ...
  useEffect(() => {
    if (data.user_id) {
      ...
      setAgronomists([]);
      setData('agronomist_id','');
    }
  }, [data.user_id]);

  const submit = (e) => {
    e.preventDefault();
    post(route('weekly-harvest-estimates.store'));
  }

  return (
    <AuthenticatedLayout user={auth?.user} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Nueva Estimación</h2>}>
        ...
        <form onSubmit={submit} className="bg-white p-6 rounded shadow">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="border rounded w-full px-2 py-2" value={data.season_code} onChange={e=>setData('season_code', e.target.value)} placeholder="T25-26"/>
