...
const Edit = ({ valor, parametros, especies }) => {
  const OPTIONAL_ESPECIE_VALUE = '__NONE__';
  const { data, setData, put, processing, errors } = useForm({
    name: valor.name ?? "",
    parametro_id: String(valor.parametro_id ?? ""),
    especie: valor.especie ?? "",
    variedad: valor.variedad ?? "",
    informe: valor.informe ?? "",
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    put(route("valores.update", valor.id));
  };

  return (
    <Card>
      <CardHeader>
        <CardTitle>Editar Valor</CardTitle>
      </CardHeader>
      <CardContent>
