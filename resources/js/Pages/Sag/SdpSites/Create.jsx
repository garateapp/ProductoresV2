...
export default function Create({ auth, csgUsers }) {
  ...
  useEffect(() => {
    // reset visible when opening or changing search
    setVisible(50);
  }, [open, search]);

  const submit = (e) => {
    e.preventDefault();
    post(route('sdp-sites.store'));
  };

  return (
    <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Nuevo SDP</h2>}>
      <Head title="Crear SDP" />
      <div className="py-12">
        <div className="max-w-3xl mx-auto sm:px-6 lg:px-8">
          <Card>
            <CardHeader>
              <CardTitle>Crear Sitio de Plantación</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={submit} className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Link href={route('sdp-sites.index')}><Button variant="outline" type="button">Cancelar</Button></Link>
                </div>
              </form>
            </CardContent>
          </Card>
