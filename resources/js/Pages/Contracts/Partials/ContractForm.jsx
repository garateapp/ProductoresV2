...
export default function ContractForm({
  ...
  useEffect(() => {
    setData('flow_redirect', !!flowRedirect);
    if (flowRut) {
      setData('flow_rut', flowRut);
    }
  }, [flowRedirect, flowRut]);
  ...
  return (
    <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="md:col-span-2 sticky bottom-0 bg-white/80 backdrop-blur border-t py-3 flex justify-between px-2 mt-2">
        {showBackLink && (
          <Link href={backHref || route('contracts.index')} className="inline-flex items-center px-4 py-2 border rounded-md">
            Volver
          </Link>
        )}
        <button type="submit" className="inline-flex items-center px-4 py-2 bg-gray-800 text-white rounded-md" disabled={processing}>
          {submitLabel}
        </button>
      </div>
    </form>
