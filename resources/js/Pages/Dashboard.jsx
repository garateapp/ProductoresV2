...
export default function Dashboard({
  ...
  const [activeQuickView, setActiveQuickView] = useState('recepciones');
  const receptionSpark = useMemo(
    () => recepciones.map((item) => item.peso_neto ?? 0),
    [recepciones]
  );
  const processSpark = useMemo(
    () => procesos.map((item) => item.kilos_netos ?? 0),
    [procesos]
  );
    ...
    >
      ...
      {!isProducer ? (
        <div className="py-12">
          <div className="mx-auto max-w-4xl rounded-2xl bg-white p-10 shadow">
            <div className="space-y-3 text-center">
              <h3 className="text-2xl font-semibold text-gray-800">Bienvenido(a)</h3>
              <p className="text-gray-600">
                Selecciona cualquiera de los módulos para comenzar o, si eres productor, solicita tu acceso para ver tu panel personalizado.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-10 py-8">
          <section
            className="relative overflow-hidden rounded-3xl border bg-gradient-to-br from-emerald-600 via-emerald-500 to-emerald-400 p-8 text-white shadow-lg"
          >
          ...
          <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <StatCard
              icon={Truck}
              title="Kg recepcionados"
              value={`${formatNumber(stats?.totalKilos)} kg`}
            />
            <StatCard
              icon={Factory}
              title="Kg procesados"
              value={`${formatNumber(stats?.totalKilosProcesados)} kg`}
            />
          </section>
